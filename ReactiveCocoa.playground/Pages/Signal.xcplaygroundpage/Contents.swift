...
scopedExample("Subscription") {
	...
	// Notice how nothing happens at this moment.
	// i.e. `subscriber2` does not receive the previously sent value.
	signal.observe(subscriber2)

	print("Send value `20` on the signal")
	// Notice that now, both `subscriber1` and `subscriber2` will receive the value.
...
scopedExample("`ignoreNil`") {
	let (signal, observer) = Signal<Int?, NoError>.pipe()
	// Note that the signal above is of type `Int?`, and the observer below is
	// of type `Int`, given we're unwrapping non-`nil` values.
...
scopedExample("`collect`") {
	let (signal, observer) = Signal<Int, NoError>.pipe()
	// Note that the signal above is of type `Int`, and the observer below is
	// of type `[Int]`, given that we're collecting `Int` values for the lifetime
	// of the signal.
	let subscriber = Observer<[Int], NoError>(next: { print("Subscriber received \($0)") } )
