...
private func enableMessageForwarding(_ objcClass: AnyClass) {
	...
	let newForwardInvocation: ForwardInvocationImpl = { object, _, invocation in
		...
		if realClass.instancesRespond(to: interopAlias) {
			...
			let interopMethod = class_getInstanceMethod(realClass, interopAlias)
			let interopImpl = method_getImplementation(interopMethod)

			if interopImpl != _rac_objc_msgForward {
				let typeEncoding = method_getTypeEncoding(interopMethod)
		...
		if perceivedClass.instancesRespond(to: selector) {
			...
			if impl != _rac_objc_msgForward {
				_ = class_replaceMethod(realClass, invokingAlias, impl, method_getTypeEncoding(method))
	...
	_ = class_replaceMethod(objcClass,
	                        ...
	                        unsafeBitCast(newForwardInvocation, to: IMP.self),
