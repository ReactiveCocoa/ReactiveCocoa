...
private func setupInterception(_ object: NSObject, for selector: Selector) -> Signal<AnyObject, NoError> {
	...
	return object.synchronized {
		...
		let hasNotBeenSwizzled = swizzledClasses
			.modify { $0.insert(ObjectIdentifier(subclass)).inserted }

		objc_sync_enter(subclass)

		let interopImpls: Box<[Selector: IMP]>
		let signatureCache: Box<[Selector: AnyObject]>

		if hasNotBeenSwizzled {
			interopImpls = Box([:])
			signatureCache = Box([:])
		...
		if signatureCache.value[selector] == nil {
			signatureCache.value[selector] = NSMethodSignature.signature(withObjCTypes: typeEncoding)
		}

		if interopImpls.value[selector] == nil {
			let immediateMethod = class_getImmediateMethod(subclass, selector)
			interopImpls.value[selector] = immediateMethod.flatMap {
