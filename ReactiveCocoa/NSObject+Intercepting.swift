...
private func setupInterception(_ object: NSObject, for selector: Selector) -> Signal<AnyObject, NoError> {
	...
	return object.synchronized {
		...
		synchronized(subclass) {
			...
			let interopImpls: Atomic<[Selector: IMP]>
			let signatureCache: Atomic<[Selector: AnyObject]>

			if isSwizzled {
				interopImpls = objc_getAssociatedObject(subclass, &interopImplKey) as! Atomic<[Selector: IMP]>
			...
			interopImpls.modify { impls in
				if impls[selector] == nil {
...
private func enableMessageForwarding(_ objcClass: AnyClass, _ interopImpls: Atomic<[Selector: IMP]>) {
	...
	let newForwardInvocation: ForwardInvocationImpl = { object, invocation in
		...
		if let interopImpl = interopImpls.withValue({ $0[selector] }) {
...
private func setupMethodSignatureCaching(_ objcClass: AnyClass, _ signatureCache: Atomic<[Selector: AnyObject]>) {
	let perceivedClass: AnyClass = class_getSuperclass(objcClass)
