...
+ (RACSignal *)switch:(RACSignal *)signal cases:(NSDictionary *)cases {
	...
	return [[[signal
		map:^(id key) {
			...
			NSCAssert(signal != nil, @"Expected %@ sent by %@ to be a key in %@", key, signal, copy);

			return signal;
		}]
		switchToLatest]
		setNameWithFormat:@"+switch: %@ cases: %@", signal, copy];
}
...
+ (RACSignal *)switch:(RACSignal *)signal cases:(NSDictionary *)cases default:(RACSignal *)defaultSignal
{
	NSCParameterAssert(signal != nil);
	NSCParameterAssert(cases != nil);
	NSCParameterAssert(defaultSignal != nil);
	...
	return [[[signal
		map:^(id key) {
			return copy[key] ?: defaultSignal;
		}]
		switchToLatest]
		setNameWithFormat:@"+switch: %@ cases: %@ default: %@", signal, copy, defaultSignal];
}
