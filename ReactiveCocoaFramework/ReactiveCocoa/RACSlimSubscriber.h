...
/// RACSlimSubscriber is a subscriber that delegates calls to the given blocks, and does nothing else.
///
/// (i.e. no synchronization, no retaining, no internal nilling on completion, no detecting notifications arriving after completion)
...
/// Returns a RACSlimSubscriber that just delegates calls to the given subscriber.
///
/// The given subscriber is returned directly if it is already a RACSlimSubscriber.
/// Avoids extra indirection when a subscriber is wrapped and tweaked several times in a chain.
+(RACSlimSubscriber*)slimSubscriberWrapping:(id<RACSubscriber>)subscriber;

/// Returns a RACSlimSubscriber with same sendCompleted and sendError blocks, but the given sendNext block.
-(RACSlimSubscriber*)withSendNext:(void(^)(id x))newSendNext;

/// Returns a RACSlimSubscriber with same sendCompleted and sendNext blocks, but the given sendError block.
-(RACSlimSubscriber*)withSendError:(void(^)(NSError* error))newSendError;

/// Returns a RACSlimSubscriber with same sendError and sendNext blocks, but the given sendCompleted block.
