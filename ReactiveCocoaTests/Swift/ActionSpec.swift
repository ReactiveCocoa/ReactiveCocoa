	...
	override func spec() {
		...
		describe("CocoaAction") {
			...
			beforeEach {
				...
				expect(action.cocoaAction.enabled).toEventually(beTruthy())
			}

			#if os(OSX)
				it("should be compatible with AppKit") {
					let control = NSControl(frame: NSZeroRect)
					control.target = action.cocoaAction
				...
				it("should be compatible with UIKit") {
					...
					control.addTarget(action.cocoaAction, action: CocoaAction.selector, forControlEvents: UIControlEvents.TouchDown)
